...
% Split into segments based on power of 2
...
% Precalculate FFTs of h segments
H2 = fft([h2;zeros(M2,1)]); % length = 64
...
outBuf8 = zeros(M,1); % for last sample of IR
...
for n=1:N
    ...
    buf8 = [outBuf8(2:end,1);h8*x(n,1)];

    % Standard convolution (one sample every loop)
    y1 = 0;
    for m=1:M1
